...
⚠️ **注意**: app.rbとserial-receiver.rbはいずれもPC上で編集してください。Raspberry Pi Pico上のフラッシュメモリで直接編集すると、書き込み回数が増加します。フラッシュメモリなどの半導体メモリには書き込み回数の上限（通常10万回程度）があるため、頻繁な書き込みを避けることでデバイスを長持ちさせることができます。
