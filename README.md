...
6. Chrome または Edge（デスクトップ，バージョン 89 以降）で [Web Terminal](https://picoruby.org/terminal) にアクセスし，シリアルポートに接続します（詳細は次章参照）．
...
⚠️ **注意**: Web Terminal の入力欄に複数行のコードを一括コピー＆ペーストすると，途中で入力が途切れることがあります．長いコードは File Editor の Upload 機能を使って配置してください（「Raspberry Pi Pico への app.rb 書き込み手順」参照）．
...
「Web Terminal への接続方法」の章を参照して，Web Terminal で Raspberry Pi Pico に接続してください．
...
1. 前述の「Raspberry Pi Pico への app.rb 書き込み手順」に従って， `/home/app.rb` にアップロードし Pico を再起動します．
