...
プログラマブルI/O（PIO）機能により、柔軟なハードウェアインターフェースを実装できます。
...
さらに、複数の機能を組み合わせて、グラフ表示とファイル記録を同時に行うプログラムに発展させることもできます。
...
PWMを使用すれば、フェードイン・フェードアウト効果や、センサー値に応じた明るさ調整など、より表現力の高いLED制御ができます。
